        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16 sm:pt-28 sm:pb-24 text-center">
          <RevealGroup>
            <RevealItem>
              ...
              <p className="font-heading font-500 text-2xl sm:text-3xl lg:text-4xl text-steel mt-2 tracking-tight">
                {c.hero.subtitle}
              </p>
            </RevealItem>
            ...
            <RevealItem>
              <AnimatePresence mode="wait" initial={false}>
                <motion.div
                  ...
                  animate="animate"
                  exit="exit"
                  className="mt-12 max-w-2xl mx-auto text-left bg-white rounded-2xl shadow-lg shadow-forest/5 border border-warm-stone/50 p-8"
                >
                  ...
                  <ul className="space-y-3">
                    {c.hero.whyName.points.map((point, i) => (
                      ...
                      >
                        <ChevronRight className="mt-1 w-4 h-4 text-sage flex-shrink-0" />
                        <span>{point}</span>
                      </li>
                    ))}
                  </ul>
                  {c.hero.whyName.bilingual && (
                    <p className="mt-6 text-ink-muted text-sm leading-relaxed border-t border-warm-stone/40 pt-4">
                      {c.hero.whyName.bilingual}
                    </p>
                  )}
                </motion.div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28">
          <RevealGroup>
            <RevealItem className="text-center mb-12">
              <span className="inline-block px-4 py-1.5 rounded-full bg-sage/20 text-sage-light text-sm font-medium tracking-wide uppercase mb-6">
            ...
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                {c.structure.items.map((item, i) => {
                  ...
                  const Icon =
                    structureIcons[i % structureIcons.length];
                  return (
                    <RevealItem key={i}>
                      <motion.div
                        whileHover={{ y: -3 }}
                        className={`rounded-2xl p-6 border ${color.bg} ${color.border} h-full`}
                      >
                        <div className="flex items-center gap-3 mb-3">
                          <div
                            className={`w-9 h-9 rounded-xl ${color.bg} border ${color.border} flex items-center justify-center`}
                          >
                            <Icon
            ...
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                {c.sessions.list.map((session, i) => {
                  const color = pastelColors[i % pastelColors.length];
                  const Icon =
                    sessionIcons[session.number] || Target;
                  return (
                    <RevealItem key={session.number}>
                      <div className="rounded-2xl bg-white border border-warm-stone/40 shadow-sm overflow-hidden">
                        <div
                          className={`${color.bg} px-6 sm:px-8 py-5 flex items-center gap-4 border-b ${color.border}`}
                        >
                          <Icon
                        ...
                        <div className="px-6 sm:px-8 py-6">
                          ...
                          <ul className="space-y-2">
                            {session.content.map((item, j) => (
                              ...
                              >
                                <span className="mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 bg-sage/60" />
                                <span>{item}</span>
                              </li>
                            ))}
                          </ul>

        ...
        {menuOpen && (
          <>
            ...
            <motion.aside
              ...
              className={`fixed top-0 right-0 z-50 h-full w-80 shadow-2xl flex flex-col transition-colors duration-300 ${
                isGallery
            ...
            >
              <div className="flex items-center justify-between px-6 py-5 border-b border-white/5">
                ...
                >
                  <Leaf className="w-5 h-5 text-sage" /> LiveWell
                </span>
                <button
                  onClick={() => setMenuOpen(false)}
                  ...
                  >
                    <h3
                      className={`px-4 text-xs font-bold uppercase tracking-widest ${
                        isGallery ? "text-white/40" : "text-ink-muted/60"
                      }`}
                    >
                      {cat.title}
                    </h3>
                    <div className="flex flex-col gap-1">
                      {cat.items.map((item, j) => (
                        <Link
                          ...
                          to={item.to}
                          onClick={() => setMenuOpen(false)}
                          className={`group flex items-center justify-between px-4 py-3.5 rounded-2xl font-semibold text-sm transition-all ${
                            isGallery
              ...
              <div
                className={`p-6 border-t ${
                  isGallery ? "border-white/5" : "border-stone/20"
                }`}
              >
                <motion.button
                  ...
                  className={`w-full flex items-center justify-center gap-2 px-4 py-3.5 rounded-2xl text-sm font-bold border-2 transition-all cursor-pointer ${
                    isGallery
                      ? "border-white/20 bg-white/10 text-white hover:bg-white/20"
                      : "border-steel/30 bg-steel-bg text-ink hover:bg-mist-bg"
                  }`}
